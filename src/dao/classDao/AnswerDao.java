...
public class AnswerDao extends JdbcDaoImpl<Answer> {
    ...
    public boolean addAnswer(Answer answer) {
        ...
        String sql = "INSERT INTO answer (user_id, task_id) " +
                "values (?,?)";
        update(connection,sql,answer.getUserId(),answer.getTaskId());
        return true;
    }
    ...
    public boolean updateAnswer(Answer answer){
        Answer expect = getAnswerByAnswerId(answer.getAnswerId());
        if (expect == null
                || !expect.getUserId().equals(answer.getUserId())
                || !expect.getTaskId().equals(answer.getUserId())) return false;
    ...
    public Answer getAnswerByAnswerId(Integer answerId){
        String sql = "SELECT answer_id answerId, user_id userId, task_id taskId, task_content taskContent " +
